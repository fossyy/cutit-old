"use client"

import { useEffect, useState } from "react";
import { SignOutModal } from "./authButton"
import axios from "axios";

interface TruncateTextProps {
    text: string;
    maxLength: number;
}

interface AuthUserProps {
    apikey: string,
    apiAddress: string,
    appAddress: string
}

export default function AddForm({ apiAddress, appAddress, apikey }: AuthUserProps) {
    const [link, setLink] = useState("")
    const [alias, setAlias] = useState("Please add some link")
    const [isSvgChanged, setIsSvgChanged] = useState(false);
    const [myLink, setMyLink] = useState([])
    useEffect(() => {
        const fetchData = async () => {
            const response = await axios.get(`${apiAddress}/api/user`, {
                params: {
                    apikey: apikey,
                },
            }); 
            if (response.data != null) {
                setMyLink(response.data);
            }
        }
        fetchData();
    }, []);

    function handleClick(e: any) {
        e.preventDefault()
        axios
            .post(`${apiAddress}/api/short`, {
                apikey: apikey,
                redirect: link,
            })
            .then((response) => {
                setAlias(`${appAddress}/${response.data.alias}`)

            })
            .catch((error) => {
                console.error("Error:", error.message);
            });
    }
    const TruncateText: React.FC<TruncateTextProps> = ({ text, maxLength }) => {
        if (text.length <= maxLength) {
            return <h1 className="mr-10">{text}</h1>;
        } else {
            const truncatedText = `${text.substring(0, maxLength)}...`;
            return <h1 className="mr-10">{truncatedText}</h1>;
        }
    }
    const copylink = () => {
        navigator.clipboard.writeText(alias)
        setIsSvgChanged(true);
        setTimeout(() => {
            setIsSvgChanged(false);
        }, 1000);
    }
    const copyRedirect = (redirect:string) => {
        navigator.clipboard.writeText(redirect)
        setIsSvgChanged(true);
        setTimeout(() => {
            setIsSvgChanged(false);
        }, 1000);
    }
    return (
        <div className="bg-slate-800 h-screen w-screen flex flex-col items-center justify-center">
            <div className="max-w-md w-full flex flex-col items-center justify-center p-6 ">
                <div id="popup-modal" className="backdrop-blur-sm fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <SignOutModal />
                </div>
                <div className="max-w-md w-full p-6 bg-slate-900 rounded-md shadow-md text-white mb-4">
                    <h1 className="text-3xl font-bold mb-4">Cut It Card</h1>
                    <div className="mb-4">
                        <h1 className="block text-sm font-medium text-gray-400 mb-1">Input Text</h1>
                        <input onChange={(e) => setLink(e.target.value)} type="url" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="https://example.com"></input>
                    </div>
                    <div className="justify-left flex flex-row">
                        <button onClick={(e) => handleClick(e)} className="bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-600 focus:outline-none focus:ring focus:border-blue-700 mr-4">Cut It</button>
                        <button data-modal-target="popup-modal" data-modal-toggle="popup-modal" className="bg-red-500 text-white py-2 px-4 rounded-md hover:bg-red-600 focus:outline-none focus:ring focus:border-blue-700">
                            Sign out
                        </button>
                    </div>
                </div>
                <div className="max-w-md w-full flex items-center justify-center p-6 bg-slate-900 rounded-md shadow-md text-white mb-4">
                    <TruncateText text={alias} maxLength={30} />
                    <button onClick={copylink}>
                        {isSvgChanged ? (
                            <svg fill="#00FF00" height="16px" width="16px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 0h24v24H0z" fill="none" />
                                <path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z" />
                            </svg>
                        ) : (
                            <svg fill="#FFFFFF" height="16px" width="16px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" enable-background="new 0 0 64 64">
                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                <g id="SVGRepo_iconCarrier">
                                    <g id="Text-files">
                                        <path d="M53.9791489,9.1429005H50.010849c-0.0826988,0-0.1562004,0.0283995-0.2331009,0.0469999V5.0228 C49.7777481,2.253,47.4731483,0,44.6398468,0h-34.422596C7.3839517,0,5.0793519,2.253,5.0793519,5.0228v46.8432999 c0,2.7697983,2.3045998,5.0228004,5.1378999,5.0228004h6.0367002v2.2678986C16.253952,61.8274002,18.4702511,64,21.1954517,64 h32.783699c2.7252007,0,4.9414978-2.1725998,4.9414978-4.8432007V13.9861002 C58.9206467,11.3155003,56.7043495,9.1429005,53.9791489,9.1429005z M7.1110516,51.8661003V5.0228 c0-1.6487999,1.3938999-2.9909999,3.1062002-2.9909999h34.422596c1.7123032,0,3.1062012,1.3422,3.1062012,2.9909999v46.8432999 c0,1.6487999-1.393898,2.9911003-3.1062012,2.9911003h-34.422596C8.5049515,54.8572006,7.1110516,53.5149002,7.1110516,51.8661003z M56.8888474,59.1567993c0,1.550602-1.3055,2.8115005-2.9096985,2.8115005h-32.783699 c-1.6042004,0-2.9097996-1.2608986-2.9097996-2.8115005v-2.2678986h26.3541946 c2.8333015,0,5.1379013-2.2530022,5.1379013-5.0228004V11.1275997c0.0769005,0.0186005,0.1504021,0.0469999,0.2331009,0.0469999 h3.9682999c1.6041985,0,2.9096985,1.2609005,2.9096985,2.8115005V59.1567993z"></path>
                                        <path d="M38.6031494,13.2063999H16.253952c-0.5615005,0-1.0159006,0.4542999-1.0159006,1.0158005 c0,0.5615997,0.4544001,1.0158997,1.0159006,1.0158997h22.3491974c0.5615005,0,1.0158997-0.4542999,1.0158997-1.0158997 C39.6190491,13.6606998,39.16465,13.2063999,38.6031494,13.2063999z"></path>
                                        <path d="M38.6031494,21.3334007H16.253952c-0.5615005,0-1.0159006,0.4542999-1.0159006,1.0157986 c0,0.5615005,0.4544001,1.0159016,1.0159006,1.0159016h22.3491974c0.5615005,0,1.0158997-0.454401,1.0158997-1.0159016 C39.6190491,21.7877007,39.16465,21.3334007,38.6031494,21.3334007z"></path>
                                        <path d="M38.6031494,29.4603004H16.253952c-0.5615005,0-1.0159006,0.4543991-1.0159006,1.0158997 s0.4544001,1.0158997,1.0159006,1.0158997h22.3491974c0.5615005,0,1.0158997-0.4543991,1.0158997-1.0158997 S39.16465,29.4603004,38.6031494,29.4603004z"></path>
                                        <path d="M28.4444485,37.5872993H16.253952c-0.5615005,0-1.0159006,0.4543991-1.0159006,1.0158997 s0.4544001,1.0158997,1.0159006,1.0158997h12.1904964c0.5615025,0,1.0158005-0.4543991,1.0158005-1.0158997 S29.0059509,37.5872993,28.4444485,37.5872993z"></path>
                                    </g>
                                </g>
                            </svg>
                        )}
                    </button>
                </div>
            </div>
            {myLink.length > 0 ? (
                <div className="max-w-md w-full p-6 bg-slate-900 rounded-md shadow-md text-white mb-4 overflow-x-auto">
                    <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                <th scope="col" className="px-6 py-3">
                                    No
                                </th>
                                <th scope="col" className="px-6 py-3">
                                    Link
                                </th>
                                <th scope="col" className="px-6 py-3">
                                    Alias
                                </th>
                                <th scope="col" className="px-6 py-3">
                                    Action
                                </th>
                            </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                            {myLink.map((item: any, index: any) => (
                                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    <td className="px-6 py-4 whitespace-nowrap">{index + 1}</td>
                                    <td className="px-6 py-4 whitespace-nowrap"><TruncateText text={item.redirect} maxLength={15} /></td>
                                    <td className="px-6 py-4 whitespace-nowrap">{item.alias}</td>
                                    <td className="px-6 py-4 whitespace-nowrap flex flex-col">
                                        <button type="button" onClick={() => copyRedirect(item.redirect)} className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Copy</button>
                                        <button type="button" className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Delete</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                </div>
            ) : (
                <div className="max-w-md w-full p-6 bg-slate-900 rounded-md shadow-md text-white mb-4 overflow-x-auto">
                    <h1>You Have No Shortlink yet try adding one</h1>
                </div>
            )}

        </div>
    )
}